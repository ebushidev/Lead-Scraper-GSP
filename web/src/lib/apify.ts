...
export function parseMaxLimitFromRow(row: Record<string, string>) {
  const maxLimitRaw = (row["Max Limit"] ?? "").trim();
  if (!maxLimitRaw) return null;
...
export function buildApifyInputFromRow(row: Record<string, string>) {
  ...
  const maxLimit = parseMaxLimitFromRow(row);
  if (maxLimit) input.maxCrawledPlacesPerSearch = maxLimit;
