...
import {
  buildApifyInputFromRow,
  getDatasetItems,
  parseMaxLimitFromRow,
  startActorRun,
  waitForRunToFinish,
} from "@/lib/apify";
...
function computeAiAnalysis(item: Record<string, unknown>) {
  let score = 2;
  ...
  if (
    categoryText.includes("agency") ||
    categoryText.includes("marketing") ||
    categoryText.includes("consult") ||
    categoryText.includes("software") ||
    categoryText.includes("technology") ||
    categoryText.includes("it ") ||
    categoryText.includes("it services") ||
    categoryText.includes("development") ||
...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      const apifyRunUrl = `https://api.apify.com/v2/actor-runs/${runState.activeRunId}`;
      await setCell(
        sheets,
        run.spreadsheetId,
        run.settingsSheetName,
      ...
      const details = [
        `Row updated.`,
        `Leads appended: ${leadsCount}.`,
        datasetUrl ? "Dataset URL saved." : null,
      ]
        .filter(Boolean)
        .join(" ");
      run.perRow.push({ row: rowNumber, status: "succeeded", datasetUrl, leads: leadsCount, message: details });
    } catch (e) {
      const finishedAt = new Date();
